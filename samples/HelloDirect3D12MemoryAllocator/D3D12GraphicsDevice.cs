...
public sealed unsafe class D3D12GraphicsDevice : IGraphicsDevice
{
        ...
        out Allocation textureUploadAllocation)
    {
        ...
        texture = _memoryAllocator.CreateResource(
            AllocationDescription.Default(HeapType.Default),
            ResourceDescription.Texture2D(Format.R8G8B8A8_UNorm, TextureSize, TextureSize, mipLevels: 1),
            ResourceStates.Common,
            out textureAllocation);
        texture.Name = "Checkerboard Texture";

        ResourceDescription textureDesc = ResourceDescription.Texture2D(Format.R8G8B8A8_UNorm, TextureSize, TextureSize, mipLevels: 1);
    ...
    private void UpdateConstantBuffers(int frame)
    {
        ...
        Matrix4x4 projection =
            Matrix4x4.CreatePerspectiveFieldOfViewLeftHanded(45.0f * (MathF.PI / 180.0f), aspectRatio, 0.1f, 1000.0f);
        Matrix4x4 view = Matrix4x4.CreateLookAtLeftHanded(new Vector3(-0.4f, 1.7f, -3.5f), Vector3.Zero, Vector3.UnitY);
        Matrix4x4 viewProjection = view * projection;

        Matrix4x4 cube1World = Matrix4x4.CreateRotationY(time);
